...
namespace Board
{
    public class MainPageViewModel : ViewModelBase
    {
        ...
        private int _totlaPage;
        public int totalPage
        {
            get { return _totlaPage; }
            set
            {
                _totlaPage = value;
                NotifyPropertyChanged();
        ...
        { 
            ...
            _isVisible = new Visibility[totalPage];
            ...
            _pageBoard = new Page[totalPage-1];
        ...
        public void bizCompleted(string temp)
        {
            ...
            {
                if (outdata.PageDisplaySEQ != null && (int)outdata.PageDisplaySEQ < totalPage)
                {
            ...
            {
                if (outdata.PageDisplaySEQ != null && outdata.ProcessDisplaySEQ != null && (int)outdata.PageDisplaySEQ < totalPage)
                {
            ...
            {
                if (!_allOfRoomStatus.ContainsKey(outdata.RoomGUID) && (int)outdata.PageDisplaySEQ < totalPage)
                {
        ...
        public void getServerData(string xmlData)
        {
            ...
            using (memStream = new MemoryStream(temp))
            {
                ...
                foreach (var rowData in ds.Tables["INDATA"].Rows)
                {
                    ...
                    //key가 존재한다면 방이 등록되어 있는 것. 방의 정보갱신
                    if (_allOfRoomStatus.ContainsKey(outdata.RoomGUID))
                    {
                        refreshRoomStatus(outdata.RoomGUID, outdata);
                    }
                    else
                    {//key가 존재 하지 않는다면 새로운 방이 생성된것. 기존에 존재하던 방의 위치에 새로운 방이 덮어 쓸 수 있으며, 새로운 위치에 방이 생성될 수 있음.
    ...
    public class Process : ViewModelBase
    {
        ...
        public Process()
        {
            ...
            roomList.Add(new RoomStatus[10]);
            for (int i = 0; i < 10; i++)
