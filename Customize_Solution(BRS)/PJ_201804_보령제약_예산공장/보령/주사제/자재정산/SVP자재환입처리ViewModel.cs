...
namespace 보령
{
    public class SVP자재환입처리ViewModel : ViewModelBase
    {
        #region [Property]
        private SVP자재환입처리 _mainWnd;
                ...
                return new CommandBase(arg =>
                    {
                        try
                        {
                            ...
                            ///
                            var viewmodel = new SVP자재환입저울스캔팝업ViewModel(enumScanType.Scale)
                            {
                                ParentVM = this,
                                parentODInfo = _mainWnd.CurrentOrder
                            };

                            SVP자재환입저울스캔팝업 ScanPopup = new SVP자재환입저울스캔팝업()
                            {
            ...
            get
            {
                return new CommandBase(arg =>
                {
                    try
                    {
                        ...
                        ///
                        var viewmodel = new SVP자재환입저울스캔팝업ViewModel(enumScanType.Quantity)
                        {
                            ParentVM = this,
                            parentODInfo = _mainWnd.CurrentOrder
                        };
        ...
        public class PackingMTRL : ViewModelBase
        {
            ...
            /// status : 자재환입 or 대기
            /// </summary>
            public void setPackingMTRL(SVP자재환입처리ViewModel vm, enumStatusType status)
            {
            ...
            public void getPackingMTRL(SVP자재환입처리ViewModel vm)
            {
