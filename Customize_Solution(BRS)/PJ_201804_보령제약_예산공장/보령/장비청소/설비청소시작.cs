...
namespace 보령
{
    ...
    [Description("설비(장비만 해당) 청소시작 액션 수행")]
    public class 설비청소시작 : ShopFloorCustomClass
    {
        public override async Task<object> ExecuteAsync(object arg)
        {
            ...
            authHelper.InitializeAsync(Common.enumCertificationType.Function, Common.enumAccessType.Create, "EM_BRS_EquipmentAction_CLEANSTART");

            if (await authHelper.ClickAsync(
                Common.enumCertificationType.Function,
                Common.enumAccessType.Create,
                string.Format("[{0}] 설비 청소시작 로그북 생성", eqptid),
                string.Format("설비 청소시작 로그북 생성"),    
                false,
                "EM_BRS_EquipmentAction_CLEANSTART",
                "", null, null) == false)
            {
            ...
            foreach (var item in inputValues)
            {
                ...
                {
                    EQACNAMECODE = "CLEANINGSTARTNAME",
                    EQPTID = item.Raw.ACTVAL != null ? item.Raw.ACTVAL : item.Raw.EQPTID,
                    USER = AuthRepositoryViewModel.GetUserIDByFunctionCode("EM_BRS_EquipmentAction_CLEANSTART"),
            ...
            this.CurrentInstruction.Raw.ACTVAL = string.Format("[{0}] 설비 청소시작 로그북 자동생성", eqptid);
