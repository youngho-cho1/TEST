...
namespace 보령
{
    ...
    [Description("설비(장비만 해당) 청소종료 액션 수행")]
    public class 설비청소종료 : ShopFloorCustomClass
    {
        public override async Task<object> ExecuteAsync(object arg)
        {
            ...
            authHelper.InitializeAsync(Common.enumCertificationType.Function, Common.enumAccessType.Create, "EM_BRS_EquipmentAction_CLEANEND");
            ...
            if (await authHelper.ClickAsync(
                ...
                false,
                "EM_BRS_EquipmentAction_CLEANEND",
                "", null, null) == false)
            {
            ...
            foreach (var item in inputValues)
            {
                ...
                {
                    EQACNAMECODE = "CLEANINGENDNAME",
                    EQPTID = item.Raw.ACTVAL != null ? item.Raw.ACTVAL : item.Raw.EQPTID,
                    USER = AuthRepositoryViewModel.GetUserIDByFunctionCode("EM_BRS_EquipmentAction_CLEANEND"),
            ...
            this.CurrentInstruction.Raw.ACTVAL = string.Format("[{0}] 설비 청소종료 로그북 자동생성", eqptid);
