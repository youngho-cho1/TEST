...
namespace 보령
{
    [Description("장비이력데이터 시작시간을 설비 사용자 속성에 저장")]
    public class 장비이력데이터시작 : ShopFloorCustomClass
    {
        ...
        public override async Task<object> ExecuteAsync(object arg)
        {
            ...
            var bizRule = new BR_BRS_MRG_HISTORYDATA_STARTTIME();
            ...
            {
                DateTime instSTARTDT;
                if (!DateTime.TryParse(equipmentRunTime[0].Raw.ACTVAL, out instSTARTDT))
                {
            ...
            {
                bizRule.INDATAs.Add(new BR_BRS_MRG_HISTORYDATA_STARTTIME.INDATA()
                {
