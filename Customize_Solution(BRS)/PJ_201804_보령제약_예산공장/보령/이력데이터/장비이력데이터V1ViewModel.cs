...
namespace 보령
{
    public class 장비이력데이터V1ViewModel : ViewModelBase
    {
                    ...
                    using (await AwaitableLocks["LoadedCommand"].EnterAsync())
                    {
                        try
                        {
                            ...
                            if (await bizrule.Execute() == true)
                            {
                                ...
                                FromDt = Convert.ToDateTime(bizrule.OUTDATAs[0].STARTTIME).AddMinutes(-5);
