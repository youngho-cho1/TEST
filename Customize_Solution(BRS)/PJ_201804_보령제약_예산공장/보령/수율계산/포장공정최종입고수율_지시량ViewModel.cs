...
namespace 보령
{
    public class 포장공정최종입고수율_지시량ViewModel : ViewModelBase
    {
        #region [Property]
        private 포장공정최종입고수율_지시량 _mainWnd;
        ...
        // 테스트용 비즈룰 수정 필요
        private BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ;
        public BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ
        {
            get { return _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ; }
            set
            {
                _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ = value;
                NotifyPropertyChanged();
            }
        }
        #endregion
                ...
                return new AsyncCommandBase(async arg =>
                {
                    using (await AwaitableLocks["LoadedCommandAsync"].EnterAsync())
                    {
                        try
                        {
                            ...
                            ///
                            if (arg != null && arg is 포장공정최종입고수율_지시량)
                            {
                                _mainWnd = arg as 포장공정최종입고수율_지시량;
                                ...
                                _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ.INDATAs.Clear();
                                _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ.OUTDATAs.Clear();

                                _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ.INDATAs.Add(new BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ.INDATA
                                {
                                ...
                                await _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ.Execute();

                                if (_BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ.OUTDATAs.Count > 0)
                                {
                                    InputVal1 = _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ.OUTDATAs[0].IN_VALUE1;
                                    InputVal2 = _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ.OUTDATAs[0].IN_VALUE2;
                                    OutputVal = decimal.TryParse(_BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ.OUTDATAs[0].OUT_VALUE, out temp) ? MathExt.Round(temp, PRECISION, MidpointRounding.AwayFromZero).ToString() : "0";
                                }
        ...
        #region [Constructor]
        public 포장공정최종입고수율_지시량ViewModel()
        {
            _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ = new BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT_REQ();
        }
        #endregion
