...
namespace 보령
{
    public class 캡슐선별공정수율ViewModel : ViewModelBase
    {
        #region Properties

        캡슐선별공정수율 _mainWndXml;
                    ...
                    using (await AwaitableLocks["LoadedCommand"].EnterAsync())
                    {
                        try
                        {
                            ...
                            if (arg != null)
                            {
                                ...
                                //캡슐선별공정수율 = (( 선별 완료 무게 (g)  ÷  캡슐 평균질량(mg/C)  × 1,000) / (캡슐 양품수량) ) * 100
