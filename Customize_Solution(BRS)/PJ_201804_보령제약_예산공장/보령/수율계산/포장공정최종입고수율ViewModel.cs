...
namespace 보령
{
    public class 포장공정최종입고수율ViewModel : ViewModelBase
    {
        #region [Property]
        private 포장공정최종입고수율 _mainWnd;
        ...
        // 테스트용 비즈룰 수정 필요
        private BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT;
        public BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT
        {
            get { return _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT; }
            set
            {
                _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT = value;
                NotifyPropertyChanged();
            }
        }
        #endregion
                ...
                return new AsyncCommandBase(async arg =>
                {
                    using (await AwaitableLocks["LoadedCommandAsync"].EnterAsync())
                    {
                        try
                        {
                            ...
                            ///
                            if (arg != null && arg is 포장공정최종입고수율)
                            {
                                _mainWnd = arg as 포장공정최종입고수율;
                                ...
                                _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT.INDATAs.Clear();
                                _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT.OUTDATAs.Clear();

                                _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT.INDATAs.Add(new BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT.INDATA
                                {
                                ...
                                await _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT.Execute();

                                if (_BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT.OUTDATAs.Count > 0)
                                {
        ...
        #region [Constructor]
        public 포장공정최종입고수율ViewModel()
        {
            _BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT = new BR_BRS_SEL_Yield_Calculation_Weight_Packing_INPUT();
        }
        #endregion
