...
namespace 보령
{
    public class 포장공정최종생산수율ViewModel : ViewModelBase
    {
        #region [Property]
        private 포장공정최종생산수율 _mainWnd;
        ...
        // 테스트용 비즈룰 수정 필요
        private BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD;
        public BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD
        {
            get { return _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD; }
            set
            {
                _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD = value;
                NotifyPropertyChanged();
            }
        }
        #endregion
                ...
                return new AsyncCommandBase(async arg =>
                    {
                        using (await AwaitableLocks["LoadedCommandAsync"].EnterAsync())
                        {
                            try
                            {
                                ...
                                ///
                                if (arg != null && arg is 포장공정최종생산수율)
                                {
                                    _mainWnd = arg as 포장공정최종생산수율;
                                    ...
                                    _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD.INDATAs.Clear();
                                    _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD.OUTDATAs.Clear();

                                    _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD.INDATAs.Add(new BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD.INDATA
                                    {
                                    ...
                                    await _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD.Execute();

                                    if (_BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD.OUTDATAs.Count > 0)
                                    {
                                        InputVal1 = _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD.OUTDATAs[0].IN_VALUE1;
                                        InputVal2 = _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD.OUTDATAs[0].IN_VALUE2;
                                        OutputVal = decimal.TryParse(_BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD.OUTDATAs[0].OUT_VALUE, out temp) ? MathExt.Round(temp, PRECISION, MidpointRounding.AwayFromZero).ToString() : "0";
                                    }
                                }
        ...
        #region [Constructor]
        public 포장공정최종생산수율ViewModel()
        {
            _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD = new BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD();
        }
        #endregion
