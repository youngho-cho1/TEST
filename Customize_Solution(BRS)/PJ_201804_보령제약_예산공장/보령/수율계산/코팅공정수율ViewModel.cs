...
namespace 보령
{
    public class 코팅공정수율ViewModel : ViewModelBase
    {
        #region Properties
        ...
        BR_BRS_SEL_Yield_Calculation_Weight_Coating _BR_BRS_SEL_Yield_Calculation_Weight_Coating;
        public BR_BRS_SEL_Yield_Calculation_Weight_Coating BR_BRS_SEL_Yield_Calculation_Weight_Coating
        {
            get { return _BR_BRS_SEL_Yield_Calculation_Weight_Coating; }
            set { _BR_BRS_SEL_Yield_Calculation_Weight_Coating = value; }
        }
                    ...
                    using (await AwaitableLocks["LoadedCommand"].EnterAsync())
                    {
                        try
                        {
                            ...
                            if (arg != null)
                            {
                                ...
                                _BR_BRS_SEL_Yield_Calculation_Weight_Coating.INDATAs.Clear();
                                _BR_BRS_SEL_Yield_Calculation_Weight_Coating.OUTDATAs.Clear();

                                _BR_BRS_SEL_Yield_Calculation_Weight_Coating.INDATAs.Add(new BR_BRS_SEL_Yield_Calculation_Weight_Coating.INDATA()
                                {
                                ...
                                await _BR_BRS_SEL_Yield_Calculation_Weight_Coating.Execute();


                                //outputLabel = "코팅 반제품 (kg)  ÷  코팅 평균질량  × 1,000,000 (T)";
                                //inputLabel = "(타정 반제품 + 코팅액 원료) (kg)  ÷  코팅 기준질량  × 1,000,000 (T)";
                                //코팅공정수율 = ( (코팅 반제품 (kg)  ÷  코팅 평균질량  × 1,000,000 (T)) / (타정 반제품 (kg)  ÷  타정 평균질량  × 1,000,000 (T)) ) * 100


                                if (_BR_BRS_SEL_Yield_Calculation_Weight_Coating.OUTDATAs.Count > 0)
                                {
                                    Result_IN = _BR_BRS_SEL_Yield_Calculation_Weight_Coating.OUTDATAs[0].IN_VALUE;
                                    Result_OUT = _BR_BRS_SEL_Yield_Calculation_Weight_Coating.OUTDATAs[0].OUT_VALUE; ;
                                    Result_SUM = _BR_BRS_SEL_Yield_Calculation_Weight_Coating.OUTDATAs[0].SUM.HasValue ? MathExt.Round(_BR_BRS_SEL_Yield_Calculation_Weight_Coating.OUTDATAs[0].SUM.Value, PRECISION, MidpointRounding.AwayFromZero) : 0m;
                                }
                            }
        ...
        public 코팅공정수율ViewModel()
        {
            _BR_BRS_SEL_Yield_Calculation_Weight_Coating = new BR_BRS_SEL_Yield_Calculation_Weight_Coating();
