...
namespace 보령
{
    public class 포장공정최종생산수율_지시량ViewModel : ViewModelBase
    {
        #region [Property]
        private 포장공정최종생산수율_지시량 _mainWnd;
        ...
        // 테스트용 비즈룰 수정 필요
        private BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG;
        public BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG
        {
            get { return _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG; }
            set
            {
                _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG = value;
                NotifyPropertyChanged();
            }
        }
        #endregion
                ...
                return new AsyncCommandBase(async arg =>
                {
                    using (await AwaitableLocks["LoadedCommandAsync"].EnterAsync())
                    {
                        try
                        {
                            ...
                            ///
                            if (arg != null && arg is 포장공정최종생산수율_지시량)
                            {
                                _mainWnd = arg as 포장공정최종생산수율_지시량;
                                ...
                                _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG.INDATAs.Clear();
                                _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG.OUTDATAs.Clear();

                                _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG.INDATAs.Add(new BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG.INDATA
                                {
                                ...
                                await _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG.Execute();

                                if (_BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG.OUTDATAs.Count > 0)
                                {
                                    InputVal1 = _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG.OUTDATAs[0].IN_VALUE1;
                                    InputVal2 = _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG.OUTDATAs[0].IN_VALUE2;
                                    OutputVal = decimal.TryParse(_BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG.OUTDATAs[0].OUT_VALUE, out temp) ? MathExt.Round(temp, PRECISION, MidpointRounding.AwayFromZero).ToString() : "0";
                                }
                            }
        ...
        #region [Constructor]
        public 포장공정최종생산수율_지시량ViewModel()
        {
            _BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG = new BR_BRS_SEL_Yield_Calculation_Weight_Packing_PROD_REG();
        }
        #endregion
