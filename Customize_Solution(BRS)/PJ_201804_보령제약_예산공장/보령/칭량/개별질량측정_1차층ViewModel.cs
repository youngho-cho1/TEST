...
namespace 보령
{
    public class 개별질량측정_1차층ViewModel : ViewModelBase
    {
        #region Property
        public 개별질량측정_1차층ViewModel()
        {
        ...
        /// 개별질량1차층 IPC저장
        /// </summary>
        private BR_BRS_REG_IPC_1ST_LAYERED_WEIGHT_MULTI _BR_BRS_REG_IPC_1ST_LAYERED_WEIGHT_MULTI;
                ...
                return new AsyncCommandBase(async arg =>
                {
                    using (await AwaitableLocks["LoadedCommand"].EnterAsync())
                    {
                        try
                        {
                            ...
                            if (arg != null && arg is 개별질량측정_1차층)
                            {
                                _mainWnd = arg as 개별질량측정_1차층;
                                _mainWnd.Closed += (s, e) =>
                                {
                                    if (_repeater != null)
                                        _repeater.Stop();
                    ...
                    using (await AwaitableLocks["ClickConfirmCommand"].EnterAsync())
                    {
                        try
                        {
                            ...
                            if (_IPC_RESULTS.Count > 0)
                            {
                                ...
                                _BR_BRS_REG_IPC_1ST_LAYERED_WEIGHT_MULTI.INDATAs.Clear();
                                ...
                                foreach (var rowdata in _IPC_RESULTS)
                                {
                                    ...
                                    _BR_BRS_REG_IPC_1ST_LAYERED_WEIGHT_MULTI.INDATAs.Add(new BR_BRS_REG_IPC_1ST_LAYERED_WEIGHT_MULTI.INDATA()
                                    {
                                        SCALEID = rowdata.SCALEID,
                                        POID = ProductionOrderInfo.OrderID,
                                        OPSGGUID = ProductionOrderInfo.OrderProcessSegmentID,
                                        SMPQTY = 1,
                                        WEIGHT = rowdata.RESULT.ToString("0.0"),
                                        SMPQTYUOMID = "",
                                ...
                                if (await _BR_BRS_REG_IPC_1ST_LAYERED_WEIGHT_MULTI.Execute() == true)
                                {
